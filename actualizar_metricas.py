...
def assign_temporal_groups(dt_arg):
    ...
    if year == 2026 and month == 8:
        ...
        # Asignación semanal genérica y escalable para cualquier mes futuro (Octubre, Noviembre, etc.)
        abbr = month_name[:3]
        if day <= 7:
        ...
        try:
            ...
            if re.search(pattern, html_code):
                ...
                with open(INDEX_OUTPUT, "w", encoding="utf-8") as f:
                    f.write(updated_html)
                print(f"[INFO] dashboard.html e index.html actualizados automáticamente.")
            else:
