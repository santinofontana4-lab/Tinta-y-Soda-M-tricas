...
foreach ($file in $csvFiles) {
    ...
    foreach ($row in $rows) {
        ...
        if ($dtArg) {
            ...
            $monthLabel = "$mName $($dtArg.Year)"

            $d = $dtArg.Day
            $m = $dtArg.Month
            $y = $dtArg.Year

            if ($y -eq 2026 -and $m -eq 8) {
                if ($d -le 16) { $weekLabel = "Semana 1 (10-16 Ago)" }
                elseif ($d -le 23) { $weekLabel = "Semana 2 (17-23 Ago)" }
                else { $weekLabel = "Semana 3 (24-31 Ago)" }
            } elseif ($y -eq 2026 -and $m -eq 9) {
                if ($d -le 7) { $weekLabel = "Semana 4 (01-07 Sep)" }
                elseif ($d -le 14) { $weekLabel = "Semana 5 (08-14 Sep)" }
                elseif ($d -le 21) { $weekLabel = "Semana 6 (15-21 Sep)" }
                else { $weekLabel = "Semana 7 (22-30 Sep)" }
            } else {
                $abbr = $mName.Substring(0, [Math]::Min(3, $mName.Length))
                if ($d -le 7) { $weekLabel = "Sem 1 (01-07 $abbr)" }
                elseif ($d -le 14) { $weekLabel = "Sem 2 (08-14 $abbr)" }
                elseif ($d -le 21) { $weekLabel = "Sem 3 (15-21 $abbr)" }
                else { $weekLabel = "Sem 4 (22-fin $abbr)" }
            }
        }
